...
function [u, k1, mu, lambda, certificate, A_matrix, b_vector, ks] = reach_avoid_controller(fx, gx, hx, vars, safe_set)
    ...
    [r_deg, Lfh, LgLfh] = vector_relative_degree(fx, gx, hx, vars);
    ...
    [b_vector, ks] = b_vector_for_backstepping(r_deg, Lfh, k1, mu, lambda, Dy_psi, p);
    % DEBUG
    ...
    certificate = compute_reach_avoid_certificate(safe_set, ks, mu, r_deg, Lfh, p);
...
% Compute reach-avoid certificate
% =====================================================
function certificate = compute_reach_avoid_certificate(safe_set, ks, mu, r_deg, Lfh, p)
...
% =====================================================
function [b, ks] = b_vector_for_backstepping(r_deg, Lfh, k1, mu, lambda, Dy_psi, p)
    ...
    for i = 1:p
        [bi, ks{i}] = bi_backstepping(r_deg, Lfh, k1, mu, lambda, Dy_psi, p, i);
        b(i) = bi;
    end
...
% =====================================================
function [bi, ks] = bi_backstepping(r_deg, Lfh, k1, mu, lambda, Dy_psi, p, output_idx)
    ...
    for j = 1:p
        ...
        h_j = Lfh{j}{1};
        Lf_h_j = Lfh{j}{2};
        jacobian_term = jacobian_term + diff(k1(output_idx), h_j) * Lf_h_j;
    end
