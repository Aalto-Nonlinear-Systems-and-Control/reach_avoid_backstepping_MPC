...
% here u is a symbolic vector [u1; u2], each entry is a symbolic expression of state variables x1,x2,x3,x4, and unknown paramet
...
samples_num = 50; % number of random samples to find the valid samples that satisfy the control input bounds for the pseudo ux

bound_min = [-1.1; -1.1; -1.1; -1.1]; % lower bounds for sampling the state space for finding valid samples that satisfy the set (safe, target, vanilla reach-avoid certificate) constraints
bound_max = [1.1; 1.1; 1.1; 1.1]; % upper bounds for sampling the state space for finding valid samples that satisfy the set (safe, target, vanilla reach-avoid certificate) constraints
...
export_to_python(u_opt, certificate_opt, params_for_export, 'sop_bounded_control_ex1_debug.py'); % export the computed controller and certificate to a python file for verification and testing
